...
class CourseController extends Controller {
    ...
    private static function authenticateAndValidateToken(Request $r, $token, $courseAlias, $assignmentAlias) {
        if (is_null($token)) {
            self::authenticateRequest($r);
            $assignmentDetails = self::validateAssignmentDetails($courseAlias, $assignmentAlias, $r['current_identity_id']);
            return [
                'hasToken' => false,
                'courseAdmin' => Authorization::isCourseAdmin($r['current_identity_id'], $assignmentDetails['course']),
    ...
    public static function apiAssignmentDetails(Request $r) {
        ...
        $tokenAuthenticationResult = self::authenticateAndValidateToken($r, $r['token'], $r['course'], $r['assignment']);
    ...
    public static function apiAssignmentScoreboard(Request $r) {
        $tokenAuthenticationResult = self::authenticateAndValidateToken($r, $r['token'], $r['course_alias'], $r['assignment_alias']);
        $group = self::resolveGroup($tokenAuthenticationResult['course']);

        if (!$tokenAuthenticationResult['hasToken'] && !Authorization::canViewCourse($r['current_identity_id'], $tokenAuthenticationResult['course'], $group)) {
            throw new ForbiddenAccessException();
        }
